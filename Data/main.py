...
# seaborn.heatmap(acc_veh_df.corr(), cmap="YlGnBu", ax=ax,)
...
features_index = np.argsort(fit.scores_)[-2:]
...
X_train, X_test, y_train, y_test = train_test_split(X[features_names], Y, test_size=0.4, random_state=0, stratify=Y)
...
# report_df.to_csv('knn/upsampling.csv', index=True)
...
# report_df = pd.DataFrame(report).transpose()
# report_df.loc['time'] = [end_time, end_time, end_time, end_time]
# report_df.to_csv('naive_bayes/upsampling.csv', index=True)
...
report_df = pd.DataFrame(report).transpose()
report_df.loc['time'] = [end_time, end_time, end_time, end_time]
report_df.to_csv('random_forest/2features.csv', index=True)
...
# report_df = pd.DataFrame(report).transpose()
# report_df.loc['time'] = [end_time, end_time, end_time, end_time]
# report_df.to_csv('svm/upsampling.csv', index=True)
...
# report_df = pd.DataFrame(report).transpose()
# report_df.loc['time'] = [end_time, end_time, end_time, end_time]
# report_df.to_csv('xgboost/upsampling.csv', index=True)
